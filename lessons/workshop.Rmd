...
1. Press `Ctrl + ,` or `Cmd + ,` to open the settings 
...
Then ask Copilot to give you code to generate a summary of the data in the `gap` data frame by plotting histograms of "lifeExp" and "gdpPercap". 
...
1. It loads `ggplot2` because it knows we need it for plotting - however, this is unnecessary because we have loaded the `tidyverse` package which includes `ggplot2` &mdash; GitHub Copilot doesn't know that. 

2. It may have set the binwidth for the histograms to 5 and 1000 - These are somewhat arbitrary values that Copilot chose and likely you as the user will want to change them. They are only somewhat arbitrary because they actually do make sense for the data; 5 and 1000 are pretty reasonable binwidths for life expectancy and GDP per capita, respectively.

3. It may have used a log scale for GDP per Capita because it knows that GDP per Capita has a wide range of values.

4. It provides guesses for the x axis title based on the name of the column in the data frame - here it has done a pretty good job! 
...
A common pitfall when working with Copilot is using it to generate code without understanding what the code is doing. **Just because code runs without an error does not mean it is correct.** Remember, you are the pilot and Copilot is...the copilot; it is crucial to use your knowledge and expertise to guide your coding decisions.

### 🥊 Challenge 7: 
To demonstrate this pitfall, let's ask Copilot to do something very broad: figure out what drives life expectancy.
...
What do you notice about this model? How is this code different than what you would have done? 

What next steps would you take to assess the validity of this model?

You may notice that Copilot recommends including variables that you may not want to include or that are colinear (for example, country and continent). Let's simplify the model:

```{r}
mod1 <- lm(lifeExp ~ gdpPercap + year + pop, data = gap)
summary(mod1)
```
...
It seems like we should fit an interaction term for country based on these plots. Let's fit a model with an interaction term for country and population size.

```{r}
mod2 <- lm(lifeExp ~ gdpPercap + year + pop:country, data = gap)
```
...
That's looking a lot better! If this was a real analysis, there are still additional steps we might want to take, but the point of this exercise is that everything we just did to validate our model was not suggested by Copilot when we asked it to identify the drivers of life expectancy. If we had just gone along with what Copilot gave us, we may not have discovered the problem with population size. We had to use our knowledge as data scientists to guide our analysis. Copilot is still helpful for creating code to run our models, generate plots, etc., but we need to be the ones in the drivers seat. You are the expert!

💡 **Tip**: If you have been going through this workshop in our GitHub Codespace and you want to save the changes you made to this file, the easiest way is to just copy and paste the file into a local file on your computer. If you are comfortable with Git, you can also fork this repository and commit your changes to your fork (instructions here: https://docs.github.com/en/codespaces/getting-started/understanding-the-codespace-lifecycle#saving-changes-in-a-codespace).

⚠️ **Final Warning**: Copilot is a wonderful tool for helping us code. It can often help us write better code and even catch common mistakes. However, it is not perfect and can provide incorrect or even harmful suggestions. It is important to always review the code that Copilot provides and make sure it is doing what you expect it to do. It is easy to slip into "Tab" completing everything and using big chunks of code written by Copilot. However, **not all incorrect code will result in a error or warning, the most common and insidious code problems are silent**, which is why it is essential to understand what your code is doing.
